...
def draw_aligned_substances(
    ...
    if template_smiles is None:
        ...
        if verbose:
            print(f"No template given, using maximum common substructure")
        ...
        if verbose:
            print(f"Using template {template_smiles}")
    ...
    depictions = []
    failed_to_align_substances = []
    for substance_index, substance in enumerate(substances):

        try:
            rdkit.Chem.AllChem.GenerateDepictionMatching2DStructure(substance, template)
        except ValueError as exception:
            print(f"ERROR: Failed to align substance '{substance_ids[substance_index]}' to template")
            print(exception)
            failed_to_align_substances.append(substance_index)
            import pdb
            pdb.set_trace()
