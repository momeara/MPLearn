...
class VirtualStainingQueue(Dataset):
    ...
    def __init__(
            self,
            subject_dataset: VirtualStainingDataset,
            max_length: int,
            samples_per_volume: int,
            sampler: tio.data.sampler.PatchSampler,
            num_workers: int = 0,
        ...
        self.patches_list: List[Tuple[tio.Subject, tio.Subject]] = []
    ...
    def __repr__(self):
        ...
        return f'VirtualStainingQueue({attributes_string})'
    ...
    def _fill(self) -> None:
        ...
        for _ in iterable:
            dcp_subject, stain_subject = self._get_next_subject()
            iterable = self.sampler(dcp_subject, stain_subject)
            patches = list(islice(iterable, self.samples_per_volume))
            self.patches_list.extend(patches)
        if self.shuffle_patches:
            random.shuffle(self.patches_list)

    def _get_next_subject(self) -> Tuple[tio.Subject, tio.Subject]:
    ...
    def get_max_memory(self, subject: Optional[tio.Subject] = None) -> int:
    ...
    def get_max_memory_pretty(self, subject: Optional[tio.Subject] = None) -> str:
